...
export default function Home() {
	...
	const isAnalyzing =
		workflow.status === "starting" || workflow.status === "pending" || workflow.status === "running"
		...
		<div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-background via-background to-muted/20">
			...
			<header className="relative z-10 flex items-center justify-between px-6 py-4">
				<div className="flex items-center gap-2">
					...
					/>
					<span className="hidden font-medium text-foreground sm:inline">Rank on AI</span>
				</div>
				<ThemeToggle />
			</header>
			...
			<main className="relative z-10 flex flex-col items-center px-4 pt-8 pb-20 sm:pt-16">
				...
				{!isAnalyzing && workflow.status !== "completed" && (
					...
					>
						...
						<p className="mt-6 max-w-xl text-lg text-muted-foreground sm:text-xl">
							Check if ChatGPT, Claude, Perplexity and other AI assistants can find and recommend
							your business.
						</p>
					</div>
				)}
									...
									return (
										<div
											...
											className={cn(
												...
												stepStatus === "running" && "bg-primary/5 text-primary",
				...
				{workflow.status === "failed" && workflow.error && (
					<div className="fade-in slide-in-from-bottom-4 w-full max-w-xl animate-in">
					...
					>
						<div className="rounded-2xl border border-border/50 bg-card/80 p-6 shadow-xl backdrop-blur-sm">
							...
							{workflow.error && workflow.status === "idle" && (
								<p className="mt-3 text-center text-destructive text-sm">{workflow.error}</p>
							)}
				...
				{!isAnalyzing && workflow.status !== "completed" && (
					...
					>
						<div className="rounded-2xl border border-border bg-card p-8">
							...
							<h2 className="font-bold text-2xl text-foreground">AI is the new search engine</h2>
							<p className="mx-auto mt-4 max-w-lg text-muted-foreground">
								More people are asking ChatGPT, Claude, and Perplexity for recommendations instead
								of searching on Google. When someone asks &quot;What&apos;s the best [your
								category]?&quot; - will your brand show up?
							</p>
							<div className="mt-8 flex flex-wrap items-center justify-center gap-3 text-sm">
								<div className="flex items-center gap-2 rounded-full border border-border bg-muted px-4 py-2 text-foreground">
