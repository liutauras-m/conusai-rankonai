...
import { Shield, Activity, Gauge, Target, CheckCircle2, Lock, Server, Zap, FileCode } from "lucide-react"
...
export default function SignalsPage() {
  ...
  const togglePreference = useCallback(
    (id: string) => {
      setPreferences((current) => (current.includes(id) ? current.filter((pref) => pref !== id) : [...current, id]))
    },
    [setPreferences]
  )
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-medium text-muted-foreground text-sm uppercase tracking-wider">Technical Signals</h2>
        </div>
          ...
          {technicalMetrics.map((metric) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-1.5 truncate font-medium text-foreground text-sm" title={metric.value}>
                  {metric.value}
                </p>
              </div>
            )
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-medium text-muted-foreground text-sm uppercase tracking-wider">Key Metrics</h2>
        </div>
        
        <div className="rounded-xl border border-border/50 bg-card p-4 sm:p-6">
          {keyMetrics.length === 0 ? (
            <p className="py-4 text-center text-muted-foreground text-sm">No additional metrics captured.</p>
          ) : (
            <div className="space-y-3">
              {keyMetrics.map(([key, value]) => (
                ...
                >
                  ...
                  <span className="font-medium text-foreground text-sm tabular-nums">{value?.toString()}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-medium text-muted-foreground text-sm uppercase tracking-wider">Optimization Priorities</h2>
        </div>
      ...
      {topFeatures.length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-medium text-muted-foreground text-sm uppercase tracking-wider">Top Strengths</h2>
          </div>
          
          {bestFeatures.length > 0 && (
            <p className="text-muted-foreground text-sm">
              {bestFeatures.map(([label, value]) => `${label.replace(/_/g, " ")}: ${value}`).join(" · ")}
            </p>
          )}
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {topFeatures.map(([label, value]) => (
              ...
              >
                ...
                <p className="mt-1.5 font-semibold text-2xl text-foreground tabular-nums">{value}</p>
