...
export async function POST(request: NextRequest) {
  try {
    ...
    const keywords = analysis.llm_context?.top_keywords?.slice(0, 10) || 
    ...
    if (type === "preferences") {
      systemPrompt = "You are an AI SEO expert. Generate user preference options based on SEO analysis. Return valid JSON only."
...
Return ONLY the JSON array, no markdown or explanation.`
      ...
      systemPrompt = "You are an AI brand strategist. Generate questions users should answer to improve AI discoverability. Return valid JSON only."
