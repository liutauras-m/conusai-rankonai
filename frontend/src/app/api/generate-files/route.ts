...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!report || typeof report !== "object") {
      return NextResponse.json({ success: false, error: "report payload is required" }, { status: 400 })
    }
    ...
    proc.stdout.on("data", (chunk) => stdoutChunks.push(typeof chunk === "string" ? chunk : chunk.toString()))
    proc.stderr.on("data", (chunk) => stderrChunks.push(typeof chunk === "string" ? chunk : chunk.toString()))
    ...
    try {
      ...
      await removeDirSafe(tempDir)
      return NextResponse.json(
        { success: false, error: "Generated files missing", detail: error instanceof Error ? error.message : String(error) },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json({ success: true, robotsContent, llmsContent, meta, stderr: stderr || undefined })
  } catch (error) {
    return NextResponse.json(
