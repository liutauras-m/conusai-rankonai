		...
		<div className={cn("prose prose-sm dark:prose-invert max-w-none", className)}>
			<ReactMarkdown
				components={{
					// Headings
					h1: ({ children }) => (
						<h1 className="mt-4 mb-2 font-bold text-foreground text-lg first:mt-0">{children}</h1>
					),
					h2: ({ children }) => (
					...
					h4: ({ children }) => (
						<h4 className="mt-2 mb-1 font-medium text-foreground text-sm first:mt-0">{children}</h4>
					),
					// Paragraphs
					p: ({ children }) => (
